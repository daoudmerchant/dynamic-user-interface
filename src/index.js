const menu = (function () {
    ...
    const displayDropDown = function (siblingBefore, dropdownLinks) {
      ...
      const fillOrClosePanel = function () {
        ...
        if (!container.classList.contains("extendDown")) {
          ...
          // panel open
          if (linkBox.classList.contains("revealGrid")) {
            // currently selected
            fadeOutAndClose(linkBox);
          } else {
            // other links displayed
            this.classList.add("selected");
    ...
    const displaySideBar = function (siblingBefore, dropdownLinks, links) {
      ...
      const toggleSidebar = function () {
        dropdownLinks.forEach((link) =>
          link.nextElementSibling.classList.remove("revealFlex")
        );
  ...
  const slideGallery = function (gallery, galleryNav) {
    ...
    // extract elements
    const icons = Array.from(galleryChildren).slice(
      1,
      galleryNav.children.length - 1
    );
    ...
    // scroll on load
    const cycleGallery = setInterval(
